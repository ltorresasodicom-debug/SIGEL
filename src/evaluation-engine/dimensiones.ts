// =============================================================================
// SIGEL — Dimensiones SIGEL en lenguaje ciudadano (capa pura)
//
// 8 dimensiones traducidas a lenguaje simple, con 3 preguntas ciudadanas cada
// una (Likert 1–5, polaridad positiva). El peso de cada pregunta dentro de su
// dimensión suma 1.0. Portado de js/data/dimensiones-ciudadanas.js.
// =============================================================================
import type { DimensionCiudadana } from './types';
import { likertA100 } from './ingel';

const LIKERT = 'likert-1-5' as const;
const W: readonly [number, number, number] = [0.34, 0.33, 0.33];

export const DIMENSIONES_CIUDADANAS: readonly DimensionCiudadana[] = [
  {
    id: 'transparencia',
    dimension: 'Transparencia',
    tituloCiudadano: 'La información está a la vista',
    descripcionCorta:
      'Qué tan fácil es enterarte de lo que hace el municipio con el dinero y las decisiones públicas.',
    descripcionExpandida:
      'Un buen gobierno local no esconde información: publica en qué gasta, qué obras hace, cuánto cuestan y quién gana los contratos. Aquí evalúas si puedes acceder a esos datos sin trabas y entenderlos.',
    ejemplo:
      'Quieres saber cuánto costó el arreglo de tu calle. Si lo encuentras publicado y en lenguaje claro, eso es transparencia.',
    interpretacionScore: {
  ...
  },
  {
    ...
    descripcionCorta:
      'Si el municipio administra con cuidado el presupuesto y no malgasta los recursos públicos.',
    descripcionExpandida:
      'El dinero del municipio viene de impuestos y transferencias del Estado. Manejarlo bien es cobrar de forma justa, gastar en lo que la gente necesita, terminar las obras y no endeudarse de más.',
    ejemplo:
      'El municipio cobra el predial y con eso termina a tiempo el parque que prometió, sin dejar deudas. Eso es buen manejo del dinero.',
    interpretacionScore: {
  ...
  },
  {
    ...
    descripcionCorta:
      'Cómo funcionan los servicios básicos que usas todos los días: agua, basura, calles, alumbrado.',
    descripcionExpandida:
      'Es lo más visible del trabajo municipal: que llegue el agua, que recojan la basura, que las vías estén en buen estado y las calles iluminadas. Aquí calificas tu experiencia real con esos servicios.',
    ejemplo:
      'El camión de la basura pasa los días que debe, el agua no se corta y tu calle está iluminada de noche. Eso son buenos servicios.',
    interpretacionScore: {
  ...
  },
  {
    ...
    descripcionCorta:
      'Si tu cantón avanza con obras útiles, buenos espacios públicos y un crecimiento bien planificado.',
    descripcionExpandida:
      'Más allá del día a día, un buen municipio piensa en el futuro: construye parques y espacios de encuentro, ordena dónde se puede construir y hace que el cantón sea un mejor lugar para vivir con el tiempo.',
    ejemplo:
      'En los últimos años aparecieron un parque nuevo y mejores veredas, y la ciudad crece sin caos. Eso es desarrollo del territorio.',
    interpretacionScore: {
  ...
  },
  {
    ...
    descripcionCorta:
      'Qué tan bien te atiende el municipio cuando necesitas un trámite o una respuesta.',
    descripcionExpandida:
      'Tarde o temprano necesitas algo del municipio: un permiso, un certificado, una respuesta. Buena gestión es que te atiendan con respeto, sin vueltas innecesarias y en un tiempo razonable.',
    ejemplo:
      'Vas a sacar un permiso, te atienden bien, te explican los pasos y lo resuelves en una sola visita. Eso es buena gestión.',
    interpretacionScore: {
  ...
  },
  {
    ...
    descripcionExpandida:
      'Las mejores decisiones se toman escuchando a la gente. Aquí evalúas si el municipio abre espacios para que opines, si toma en serio lo que dicen los vecinos y si puedes influir en lo que pasa en tu barrio.',
    ejemplo:
      'El municipio reúne al barrio para decidir en qué usar el presupuesto y de verdad toma en cuenta lo que la gente propone. Eso es participación.',
    interpretacionScore: {
  ...
  },
  {
    ...
    descripcionCorta:
      'Cuánta confianza te generan las autoridades del municipio y su forma de actuar.',
    descripcionExpandida:
      'La confianza se gana cumpliendo lo prometido y actuando con honestidad. Aquí evalúas si crees que tus autoridades trabajan para la gente, cumplen su palabra y manejan lo público con rectitud.',
    ejemplo:
      'El alcalde prometió arreglar el mercado y lo cumplió tal como dijo. Eso construye confianza.',
    interpretacionScore: {
  ...
  },
  {
    ...
    descripcionCorta:
      'Si el municipio te facilita la vida con herramientas digitales: web, trámites en línea, apps.',
    descripcionExpandida:
      'Hoy muchas cosas se resuelven desde el celular. Un municipio moderno tiene una web útil, permite hacer trámites o pagos en línea y usa medios digitales para informar, evitándote filas y viajes.',
    ejemplo:
      'Pagas el predial o sacas un certificado desde el celular, sin ir a hacer fila. Eso es innovación digital.',
    interpretacionScore: {
...
export const TOTAL_PREGUNTAS = DIMENSIONES_CIUDADANAS.reduce(
  (n, d) => n + d.preguntas.length,
  0,
);

/** Score 0–100 de una dimensión = media ponderada de sus preguntas. */
export function agregarDimension(
  dim: DimensionCiudadana,
  respuestas: Record<string, number | null | undefined>,
): number {
  let s = 0;
  for (const q of dim.preguntas) s += likertA100(respuestas[q.id] ?? null) * q.peso;
  return s;
}
